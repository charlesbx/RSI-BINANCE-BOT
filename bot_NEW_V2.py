...
-0.5% -> +0.15% | -1% -> -0.75%<br>\
EXTREME COND 3hrs -2%<br>\
-> -1% | +1h -> -1.5% | +2h -> -2% | +3h -> -8%<br>\
WIN WITH RSI : +0.75%<br>\
WIN NO RSI : +3%<br>\
LOWESTRSI for buy<br>\
SELL_TIME + 10minutes for buy<br>\
if last_rsi > RSI_OVERBOUGHT then i = 0<br>\
LAST_RSI + 3 <= highest_rsi<br>"
...
def buy_writer():
    ...
    bad_trade_count, start_price, MovementNbr, timeElapsed, spended, low_price, top_price
    ...
    reportFile.write("[" + datetime.now().strftime("%H:%M|%b%d") + "]Lowest Price + 0% : " + str(round(low_price, 2)) + "\n")
    reportFile.write("[" + datetime.now().strftime("%H:%M|%b%d") + "]Highest Price - 0.75% :" + str(round(top_price, 2)) + "\n")
    ...
    emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]Lowest Price + 0% : " + str(round(low_price, 2)) + "<br>"
    emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]Highest Price - 0.75% :" + str(round(top_price, 2)) + "<br>"
...
def on_message(ws, message):
    global closes, in_position, was_closed, walletUSD, walletETH, last_buy_close, i, \
    last_sell_price, close, emailContent, buy_time, symbol1, symbol2, last_time_check, minute_limit, \
    bad_trade_count, start_price, MovementNbr, timeElapsed, sell_time, spended, html, \
    low_price, temp_lowest, price_swap, price_swap2, temp_top, top_price, sell_at_buyprice, sell_fast, highest_price, lowest_price, \
    endASAP, highest_rsi, lowest_rsi, sellVeryFast, sellVeryFastTime, veryFastLoseAmt, dataFile
...
########################################################################################################################################################################
        ...
        if in_position and float(close) <= last_buy_close - percent.percentage(1, last_buy_close) and buy_time + timedelta(hours=2) <= datetime.now():
            sell_fast = False
        if in_position and float(close) <= last_buy_close - percent.percentage(2, last_buy_close) and buy_time + timedelta(hours=3) <= datetime.now():
            sellVeryFast = True
            sellVeryFastTime = datetime.now()
            veryFastLoseAmt = 1
        if in_position and sellVeryFast:
            if sellVeryFastTime >= datetime.now() - timedelta(hours=1):
                veryFastLoseAmt = 1.5
            if sellVeryFastTime >= datetime.now() - timedelta(hours=2):
                veryFastLoseAmt = 2
            if sellVeryFastTime >= datetime.now() - timedelta(hours=3):
                veryFastLoseAmt = 8
        if in_position and sellVeryFast and buy_time + timedelta(hours=6) <= datetime.now() and last_rsi > RSI_OVERBOUGHT and rsi[-1] > rsi[-3]:
            TRADE_QUANTITY = walletETH
            last_sell_price = float(close)
            bad_trade_count += 1
            ...
            reportFile.write("[" + datetime.now().strftime("%H:%M|%b%d") + "]HARD LOSED - " + str(round((last_buy_close * float(TRADE_QUANTITY)) - walletUSD, 2)) + "$! | - " + str(round(((last_buy_close * float(TRADE_QUANTITY)) - walletUSD) / (TRADE_QUANTITY * last_buy_close) * 100, 2)) + "%!" + '\n')
            reportFile.write("[" + datetime.now().strftime("%H:%M|%b%d") + "]" + "Begin Time : " + buy_time.strftime("%H:%M:%S") + '\n')
            reportFile.write("[" + datetime.now().strftime("%H:%M|%b%d") + "]" + "End Time : " + datetime.now().strftime("%H:%M:%S") + "\n")
            reportFile.write("[" + datetime.now().strftime("%H:%M|%b%d") + "]" + "Time Elapsed : " + str(timeElapsed).split('.')[0] + '\n')
            ...
            emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]Spend : " + str(round(TRADE_QUANTITY, 4)) + " " + symbol1.upper() + '<br>'
            emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]Sell Price :" + str(round(last_sell_price, 3)) + " " + symbol2.upper() + "<br>"
            emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]HARD LOSED - " + str(round((last_buy_close * float(TRADE_QUANTITY)) - walletUSD, 2)) + "$! | - " + str(round(((last_buy_close * float(TRADE_QUANTITY)) - walletUSD) / (TRADE_QUANTITY * last_buy_close) * 100, 2)) + "%!" + '<br>'
            emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]" + "Begin Time : " + buy_time.strftime("%H:%M:%S") + "<br>"
            emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]" + "End Time : " + datetime.now().strftime("%H:%M:%S") + "<br>"
            emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]" + "Time Elapsed : " + str(timeElapsed).split('.')[0] + "<br>"
        ...
        (in_position and sellVeryFast and float(close) >= last_buy_close - percent.percentage(veryFastLoseAmt, last_buy_close))):
            TRADE_QUANTITY = walletETH
            last_sell_price = float(close)
            bad_trade_count += 1
            order_succeeded = fake_order(SIDE_SELL, close)
...
############################################################################## BUY WITH RSI ############################################################################
########################################################################################################################################################################
                if (i > 200 and float(close) <= top_price) or float(close) <= low_price: #and float(close) <= top_price:
                    ...
                    if last_rsi - 5 >= lowest_rsi:
                        ...
                        # emailContent += "[" + datetime.now().strftime("%H:%M|%b%d") + "]Lowest Price + 0% : " + str(round(low_price, 2)) + "\n"
